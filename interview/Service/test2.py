...
tools = [{
    ...
    "name": "get_random_cat",
    "function": {
        "description": "Get random cat images from TheCatAPI",
        "parameters": {
            "type": "object",
            "properties": {
                "limit": {
...
def get_random_cat(limit=1):
    ...
    for cat in data:
        ...
        if "breeds" in cat and cat["breeds"]:
            breed = cat["breeds"][0]
            item.update({
                "name": breed["name"],
            ...
            item["text"] = "Here's a cute random cat!"
        results.append(item)
...
def chat_with_function(user_input, room_id, prev_response_id=None):
    ...
    for response_out in response.output:
        if response_out.type == "function_call":
            res.append(response_out)

    if len(res) > 0:
        result = get_random_cat(len(res))
